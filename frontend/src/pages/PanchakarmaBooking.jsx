...
const PanchakarmaBooking = () => {
    ...
    const mockDoctors = [
        ...
        },
        {
            id: 3,
            name: 'Dr. Meera Patel',
            specialization: 'Dosha Analysis Expert',
            experience: '12 years',
            rating: 4.7,
    ...
    const mockCenters = [
        ...
        },
        {
            ...
            facilities: ['Traditional Therapy Rooms', 'Yoga Studio', 'Organic Kitchen'],
            rating: 4.8
        },
        {
            id: 3,
            ...
            facilities: ['Modern Equipment', 'Expert Therapists', 'Consultation Rooms'],
            rating: 4.5
        }
    ];
    ...
    const doshaTypes = [
        ...
        { id: 'mixed', name: 'Mixed Constitution', description: 'Combination of doshas' }
    ];
    ...
    const getRecommendedTherapy = (dosha) => {
        const recommendations = {
            vata: { name: 'Basti (Medicated Enema)', description: 'Balances Vata dosha and strengthens nervous system' },
            pitta: { name: 'Virechana (Purgation)', description: 'Cleanses liver and balances Pitta dosha' },
            kapha: { name: 'Vamana (Therapeutic Vomiting)', description: 'Removes excess Kapha and respiratory toxins' },
            mixed: { name: 'Comprehensive Panchakarma', description: 'Customized multi-therapy approach' }
        };
    ...
    const handleInputChange = (field, value) => {
        ...
        if (field === 'selectedDate' && formData.doshaType) {
            fetchTimeSlots(value, formData.selectedDoctor.id, formData.selectedCenter.id, formData.doshaType);
        }

        if (field === 'doshaType' && formData.selectedDate) {
            fetchTimeSlots(formData.selectedDate, formData.selectedDoctor.id, formData.selectedCenter.id, value);
        }
    };
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="text-center mb-12">
                        ...
                        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                            Book your personalized Panchakarma therapy session with our expert practitioners
                        </p>
                    </div>
                        ...
                        <div className="flex items-center justify-center space-x-8">
                            {[1, 2, 3, 4].map((stepNumber) => (
                                <div key={stepNumber} className="flex items-center">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${step >= stepNumber
                                            ? 'bg-green-600 text-white'
                                            : 'bg-gray-200 text-gray-600'
                                        }`}>
                                        {stepNumber}
                                    </div>
                                    <span className={`ml-2 text-sm ${step >= stepNumber ? 'text-green-600' : 'text-gray-500'
                                        }`}>
                    ...
                    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl p-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                        {doshaTypes.map((dosha) => (
                                            <div
                                                ...
                                                className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${formData.doshaType === dosha.id
                                                        ? 'border-green-500 bg-green-50'
                                                        : 'border-gray-200 hover:border-green-300'
                                                    }`}
                                            >
                                                <h3 className="font-semibold text-gray-800">{dosha.name}</h3>
                                                <p className="text-sm text-gray-600 mt-1">{dosha.description}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        {step === 2 && (
                            <div className="space-y-8">
                                <h2 className="text-2xl font-bold text-gray-800 mb-6">Select Doctor & Center</h2>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {doctors.map((doctor) => (
                                            <div
                                                ...
                                                className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${formData.selectedDoctor?.id === doctor.id
                                                        ? 'border-green-500 bg-green-50'
                                                        : 'border-gray-200 hover:border-green-300'
                                                    }`}
                                            >
                                                <div className="flex items-center space-x-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-500">{doctor.experience} • ⭐ {doctor.rating}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-4">
                                        Choose Treatment Center <span className="text-red-500">*</span>
                                    </label>
                                    <div className="space-y-4">
                                        {centers.map((center) => (
                                            <div
                                                ...
                                                className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${formData.selectedCenter?.id === center.id
                                                        ? 'border-green-500 bg-green-50'
                                                        : 'border-gray-200 hover:border-green-300'
                                                    }`}
                                            >
                        ...
                        {step === 3 && (
                            <div className="space-y-8">
                                ...
                                {recommendedTreatment && (
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                                        <div className="flex items-start space-x-3">
                                            ...
                                            <div>
                                                <h3 className="font-semibold text-green-800">Recommended for your Dosha</h3>
                                                <p className="text-green-700 mt-1">
                                                    <span className="font-medium">{recommendedTreatment.therapy.name}</span> - {recommendedTreatment.therapy.description}
                                                </p>
                                                <p className="text-sm text-green-600 mt-2">
                                                    Duration: {recommendedTreatment.duration} • Sessions: {recommendedTreatment.sessions}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            {availableDates.map((date) => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`p-3 border-2 rounded-lg cursor-pointer text-center transition-all ${formData.selectedDate === date
                                                                ? 'border-green-500 bg-green-50'
                                                                : 'border-gray-200 hover:border-green-300'
                                                            }`}
                                                    >
                                        ...
                                        {loading ? (
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                                {availableSlots.map((slot) => (
                                                    <div
                                                        key={slot.time}
                                                        onClick={() => slot.available && handleInputChange('selectedTimeSlot', slot)}
                                                        className={`p-4 border-2 rounded-lg transition-all ${!slot.available
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={!formData.selectedDate || !formData.selectedTimeSlot || loading}
